...
#include "nrf_socket.h"
			...
			memcpy(ifr.ifr_name, local->interface, strlen(local->interface));

			int err = nrf_setsockopt(socket_fd, NRF_SOL_SOCKET, NRF_SO_BINDTODEVICE, 
							(void *)&ifr, strlen(local->interface));
			if (err == -1) {
